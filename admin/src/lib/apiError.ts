...
const STATUS_FALLBACK: Record<number, StatusMessageMap> = {
  ...
  401: { zh: "登录状态已过期，请重新登录", en: "Session expired. Please sign in again." },
  403: { zh: "没有权限执行此操作", en: "You do not have permission to do that." },
  404: { zh: "资源不存在或已被删除", en: "The resource no longer exists." },
  409: { zh: "资源冲突（例如名称重复）", en: "Resource conflict (e.g. duplicate name)." },
  413: { zh: "上传文件过大", en: "File too large." },
  422: { zh: "提交内容不合法", en: "Submitted content is invalid." },
  429: { zh: "请求过于频繁，请稍后再试", en: "Too many requests. Try again shortly." },
  500: { zh: "服务器内部异常，请稍后重试", en: "Internal server error. Try again shortly." },
  502: { zh: "后端网关异常", en: "Bad gateway." },
  503: { zh: "服务暂不可用，请稍后重试", en: "Service unavailable. Try again shortly." },
...
export function buildApiError(
  ...
  const friendly = backendMessage || STATUS_FALLBACK[response.status];
  const message =
    friendly ?? `请求失败（${response.status}） / Request failed (${response.status})`;
  return new ApiError({
    message,
