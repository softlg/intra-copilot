...
After each requested change is complete, create a commit without waiting for a separate commit request once all applicable verification passes. Do not commit while checks are failing or the task is incomplete. Stage only files that belong to the current task, and leave unrelated working-tree changes untouched. After a successful commit, push the current branch to its remote upstream without waiting for a separate push request. If the branch has no upstream, push it with `git push -u origin <branch>`. Never force-push. If the push fails, report the failure and do not claim the work has been delivered.
