...
public class SystemAgentSynchronizer {
    ...
    private void synchronize(SystemAgentCatalog.Spec spec) {
        ...
        long nextVersion =
                versions.findByAgentId(spec.id())
                                .stream()
    ...
    private void disableLegacyBrowserConfiguration() {
        ...
        for (AgentDefinition agent : new ArrayList<>(definitions.findAll())) {
            ...
            if (releaseNeedsMigration || (agent.isPublished() && agent.getPublishedVersion() > 0)) {
                publishSanitizedRelease(agent, browserToolIds, browserSkillIds);
            }
        }
    }
    ...
    private void publishSanitizedRelease(
            AgentDefinition agent, Set<String> browserToolIds, Set<String> browserSkillIds) {
        ...
        long next =
                existingVersions.stream().mapToLong(AgentConfigVersion::getVersion).max().orElse(0L)
                        + 1L;
        ...
        release.setSnapshot(
                snapshotCodec.encode(
                        base, decoded.structured() ? decoded.childBindings() : List.of()));
        versions.save(release);
    ...
    private List<String> parseIds(String raw) {
        ...
        try {
            List<String> values = json.readValue(raw, new TypeReference<List<String>>() {});
