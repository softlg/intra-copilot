...
import com.intra.copilot.infrastructure.knowledge.EmbeddingSchema;
import com.intra.copilot.infrastructure.persistence.knowledge.EmbeddingProfileRepository;
...
public class EmbeddingProfileService {
    ...
    public List<EmbeddingProfile> listEnabled() {
        return profiles.findAll().stream().filter(EmbeddingProfile::isEnabled).toList();
    }

    public List<EmbeddingProfile> listAll() {
        return profiles.findAll();
    }

    public EmbeddingProfile get(String id) {
        return profiles.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Embedding 配置不存在"));
    }
    ...
    public EmbeddingProfile resolve(KnowledgeBase base) {
        ...
        if (base != null
                && Boolean.FALSE.equals(base.getUseSystemEmbedding())
                && hasCustomEmbedding(base)) {
            return customProfile(base);
        }
        if (base != null
                && base.getEmbeddingProfileId() != null
                && !base.getEmbeddingProfileId().isBlank()) {
            EmbeddingProfile profile = get(base.getEmbeddingProfileId());
            if (!profile.isEnabled())
                throw new IllegalStateException("Embedding 配置已停用：" + profile.getName());
            return profile;
        }
    ...
    private boolean hasCustomEmbedding(KnowledgeBase base) {
        return base.getEmbeddingProvider() != null
                && !base.getEmbeddingProvider().isBlank()
                && base.getEmbeddingModel() != null
                && !base.getEmbeddingModel().isBlank()
                && base.getEmbeddingDimension() != null
                && base.getEmbeddingDimension() > 0;
    }
    ...
    private EmbeddingProfile customProfile(KnowledgeBase base) {
        ...
        profile.setBaseUrl(base.getEmbeddingBaseUrl());
        profile.setModel(base.getEmbeddingModel());
        profile.setApiKey(base.getEmbeddingApiKey());
    ...
    public EmbeddingProfile builtInDefault() {
        EmbeddingProfile profile = new EmbeddingProfile();
        profile.setId("system-default-embedding");
        profile.setName("运行时默认");
        profile.setProvider(defaultProvider);
        profile.setModel(defaultModel);
        profile.setDimension(defaultDimension);
        profile.setConfigVersion("runtime");
        profile.setDefaultProfile(true);
        return profile;
    }
    /**
     * Request body for {@link #applyConfig}: either inherit the system model, supply a custom one,
     * or (legacy) pick a centrally managed profile by id.
     */
    ...
    public EmbeddingProfile resolveByKnowledgeBaseId(String id) {
        return resolve(
                bases.findById(id).orElseThrow(() -> new IllegalArgumentException("知识库不存在")));
    }

    public EmbeddingProfile save(EmbeddingProfile profile) {
        if (profile.getName() == null || profile.getName().isBlank())
            throw new IllegalArgumentException("Embedding 配置名称不能为空");
        if (profile.getProvider() == null || profile.getProvider().isBlank())
                    ...
                    .forEach(
                            item -> {
                                if (!item.getId().equals(profile.getId())
                                        && item.isDefaultProfile()) {
                                    item.setDefaultProfile(false);
                                    profiles.updateById(item);
                                }
                            });
        return profiles.save(profile);
    }

    public EmbeddingProfile configForBase(String baseId) {
        return resolveByKnowledgeBaseId(baseId);
    }

    public EmbeddingProfile setForBase(String baseId, String profileId) {
        return applyConfig(
                baseId, new EmbeddingConfigRequest(null, null, null, null, null, null, profileId));
    }
    ...
    public EmbeddingProfile applyConfig(String baseId, EmbeddingConfigRequest request) {
        ...
        if (request.useSystemEmbedding() != null && request.useSystemEmbedding()) {
            ...
            base.setEmbeddingBaseUrl(null);
            base.setEmbeddingModel(null);
            base.setEmbeddingApiKey(null);
            ...
            // Custom models use this knowledge base's connection details instead of provider env.
            ...
            if (apiKey == null || apiKey.isBlank()) {
                ...
                if (apiKey == null || apiKey.isBlank()) {
                    throw new IllegalArgumentException("首次配置自定义模型时 API Key 不能为空");
                }
            }
            base.setUseSystemEmbedding(false);
            base.setEmbeddingProvider(provider.trim());
            base.setEmbeddingBaseUrl(baseUrl.trim());
            base.setEmbeddingModel(model.trim());
            base.setEmbeddingApiKey(apiKey.trim());
