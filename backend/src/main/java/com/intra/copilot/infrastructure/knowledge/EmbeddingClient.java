...
public class EmbeddingClient {
    ...
    public List<Double> embed(String text) {
        return embed(text, profiles.builtInDefault());
    }

    public List<Double> embed(String text, EmbeddingProfile profile) {
        rateLimiter.acquire(profile.getProvider());
        int maxAttempts =
                Math.max(1, environment.getProperty("embedding.max-attempts", Integer.class, 3));
        long backoffMs =
                Math.max(
                        200,
    ...
    private List<Double> call(String text, EmbeddingProfile profile) {
        ...
        String baseUrl =
                hasText(profile.getBaseUrl())
                        ? profile.getBaseUrl().trim()
        ...
        String apiKey =
                hasText(profile.getApiKey())
                        ? profile.getApiKey().trim()
                        : environment.getProperty(
                                prefix + ".api-key",
                                environment.getProperty(
        ...
        try {
            ...
            if (values.size() != profile.getDimension())
                throw new IllegalStateException(
                        "Embedding 维度不匹配：配置 " + profile.getDimension() + "，实际 " + values.size());
            ...
            if (retryable)
                throw new RetryableEmbeddingException(
                        "HTTP " + status + " " + error.getMessage(), error);
            throw new IllegalStateException(
                    "Embedding 配置「" + profile.getName() + "」调用失败：HTTP " + status, error);
            ...
            throw new IllegalStateException(
                    "Embedding 配置「" + profile.getName() + "」调用失败：" + error.getMessage(), error);
        }
    }
