...
public interface EmbeddingProfileRepository extends BaseMapper<EmbeddingProfile> {
    ...
    default Optional<EmbeddingProfile> findById(String id) {
        return Optional.ofNullable(selectById(id));
    }

    default List<EmbeddingProfile> findAll() {
        return selectList(Wrappers.<EmbeddingProfile>query().orderByAsc("name"));
    }

    default Optional<EmbeddingProfile> findDefault() {
        return Optional.ofNullable(
                selectOne(
                        Wrappers.<EmbeddingProfile>query()
