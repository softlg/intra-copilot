package com.intra.copilot.shared.util;
 ...
 *
 * <p>Each ID is exactly 13 characters: a two-character module code followed by an eleven-character,
 * zero-padded Base58 Snowflake value. The Snowflake layout is 41 bits of timestamp, 5 bits of node
 * ID, and 12 bits of per-millisecond sequence.
 *
 * <p>Set {@code APP_NODE_ID} (or {@code -Dapp.node-id}) to a unique value from 0 to 31 for every
 * backend instance. When omitted, a node number is derived from the host and JVM identity so local
 * instances do not all use the same value.
 */
