 ...
 * 设备公钥表。
 *
 * <p>{@code publicKeyJwk} 是设备上传的 RSA 公钥 JWK；数据库列为 {@code json} 类型， 借助 MyBatis-Plus 内置的 自定义
 * PostgreSQL JSON 类型处理器 在写入时自动把 {@link JsonNode} 序列化为合法 JSON、读取时再反序列化回来。
 */
...
public class DeviceKey {
    ...
    public JsonNode getPublicKeyJwk() {
        return publicKeyJwk;
    }

    public void setPublicKeyJwk(JsonNode v) {
        publicKeyJwk = v;
    }
