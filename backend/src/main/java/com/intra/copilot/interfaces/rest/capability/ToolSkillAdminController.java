...
public class ToolSkillAdminController {
    ...
    public ToolDefinition updateTool(@PathVariable String id, @RequestBody ToolDefinition t) {
        ...
        if (isSystemBrowserTool(current)) {
            throw new IllegalArgumentException("浏览器操作能力由系统内置 Browser Operator 提供，不支持后台修改");
        }
        validateTool(t);
    ...
    public ToolDefinition toggleTool(@PathVariable String id, @RequestBody EnabledRequest request) {
        ...
        if (isSystemBrowserTool(tool)) {
            throw new IllegalArgumentException("浏览器操作能力由系统内置 Browser Operator 提供，不支持后台启停");
        }
    ...
    public Map<String, Object> testTool(
        ...
        ToolDefinition tool =
                tools.findById(id).orElseThrow(() -> new NoSuchElementException("Tool 不存在：" + id));
        if (isSystemBrowserTool(tool)) {
            throw new IllegalArgumentException("浏览器操作能力由系统内置 Browser Operator 提供，不支持后台测试配置");
        }
    ...
    public void deleteTool(@PathVariable String id) {
        ...
        ToolDefinition tool =
                tools.findById(id).orElseThrow(() -> new IllegalArgumentException("Tool 不存在"));
        if (isSystemBrowserTool(tool)) {
            throw new IllegalArgumentException("浏览器操作能力由系统内置 Browser Operator 提供，历史记录仅保留用于审计");
        }
        tools.deleteById(id);
    }
